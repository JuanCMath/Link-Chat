...
class FTv2:
    ...
    def send_file(self, dst_mac: bytes, local_path: str) -> None:
        """
        ...
        self._send_ctrl(
            dst_mac, {"op": "REQ", "sid": sid, "name": self.name, "fn": fn, "sz": sz}
        )
    ...
    def _on_data(self, src_mac: bytes, seq: int, chunk: bytes) -> None:
        ...
        if self.rx["recv"] >= self.rx["sz"]:
            try:
                ...
                pass
            self._send_ctrl(src_mac, {"op": "DONE", "sid": self.rx["sid"]})
            self.on_info(f"[ft] RX DONE sid={self.rx['sid']} -> {self.rx['path']}")
            self.on_complete("rx", self.rx["sid"], True)
            self.rx = None
        ...
        with self._tx_lock:
            ...
            if total == 0:
                ...
                if drop_kind is None:
                    ...
                    if fh is None:
                        drop_kind = "fail"
                        drop_exc = RuntimeError("no se pudo abrir el archivo para lectura")
                    else:
        ...
        if kind == ACK_KIND_DATA:
            ...
            if sid and ack_value is not None:
                ...
                if meta:
                    ...
                    self.on_info(
                        f"[ft] ACK data sid={sid} seq={ack_value} de {self._mac2s(src_mac)} (intent={attempts})"
                    )
                    acked = 0
                    total = 0
                    fn = ""
