...
def ft_on_complete(role: str, sid: str, ok: bool) -> None:
    ...
    archive_path = pending_archives.pop(sid, None)
    if archive_path:
        try:
            os.remove(archive_path)
        except OSError as exc:
            print(f"[senddir] cleanup failed for {archive_path}: {exc}", flush=True)
        ...
        if sys.stdin.isatty():
            while True:
                ...
                if line.startswith("/senddir "):
                    ...
                    if not mac_str:
                        print(
                            f"[senddir] peer '{token}' not found. Use /peers.",
                            flush=True,
                        )
                        continue
                    ...
                    try:
                        ...
                        print("[senddir] Invalid MAC.", flush=True)
                        try:
                            os.remove(archive_path)
                        except OSError:
                            pass
                        continue
                    print(
                        f"[senddir] sending {folder_name}/ as {archive_name}",
                        flush=True,
                    )
                    sid = ft.send_file(
                        dst_mac,
                    ...
                    if not sid:
                        try:
                            ...
                            pass
                        continue
                    pending_archives[sid] = archive_path
                    continue
